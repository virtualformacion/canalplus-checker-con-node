...
// ? trySelectors con timeout corto pero comportamiento original preservado
...
async function procesarCombo(combo, index, total, validCount, invalidCount) {
  ...
  while (reintentar) {
    ...
    try {
      ...
      await page.waitForTimeout(100); // ?? tiempo restaurado
      ...
      if (loginResult === 'success') {
        ...
        await success.click();
        await page.waitForTimeout(17000); // ?? tiempo restaurado
...
(async () => {
  ...
  for (let i = 0; i < combos.length; i++) {
    ...
    await new Promise(resolve => setTimeout(resolve, 500)); // ?? delay restaurado
  }
